...
def interpolate_ms_features(pts: torch.Tensor,
    ...
    for scale_id, grid in enumerate(ms_grids[:num_levels]):
        ...
        for ci, coo_comb in enumerate(coo_combs):
            ...
            # if not torch.isfinite(torch.as_tensor(interp_out_plane)).all():
                #  print('CP022: Nan Found')
                #  a=0
                #  b=0
    ...
    if not torch.isfinite(torch.as_tensor(multi_scale_interp)).all():
        print('CP023: Nan Found')
        a=1
        b=1
...
#           sigma_net just does a linear transformation on the features to get density
    ...
    def get_density(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None):
        ...
        if self.linear_decoder:
            ...
            # density_before_activation = features[..., -1:]
            # features = features[...,:-1]
        else:
    ...
    def forward(self,
        ...
        if not self.linear_decoder:
            directions = get_normalized_directions(directions)   # TODO : will work fine with ndc
...
class disp_KPlaneField(nn.Module):
    def __init__(
        self,
        aabb,
        grid_config: Union[str, List[Dict]],
...
#    shape[1] is out-dim - Concatenate over feature len for each scale
            if self.concat_features:
                ...
                self.feature_dim = gp[-1].shape[1]
            self.disp_grids.append(gp)
        self.disp_net = tcnn.Network(
            n_input_dims=self.feature_dim,
            n_output_dims=3,
            network_config={
                "otype": "CutlassMLP",
        ...
        log.info(f"Initialized displacement grids: {self.disp_grids}")

    def get_disp(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None):
        pts = normalize_aabb(pts, self.aabb)
        n_rays, n_samples = pts.shape[:2]
        if timestamps is not None:
            timestamps = timestamps[:, None].expand(-1, n_samples)[..., None]  # [n_rays, n_samples, 1]
        ...
        if not torch.isfinite(pts).all():
            print('CP024: Nan Found')
            a=1
            b=2
        features = interpolate_ms_features(
            pts, ms_grids=self.disp_grids,  # noqa
            grid_dimensions=self.grid_config[0]["grid_dimensions"],
            concat_features=self.concat_features, num_levels=None, in_dim=self.grid_config[0]["input_coordinate_dim"]) #, time_planes_only=True

        if len(features) < 1:
            features = torch.zeros((0, 1)).to(features.device)
        if not torch.isfinite(features).all():
            print('CP025: Nan Found')
            a = 0
            b = 1
        if self.disp_net.n_input_dims != features.shape[-1] and features != None:
            print("CP30: feature dimentions are not matching")
        disp = self.disp_net(features).to(features)
        if not torch.isfinite(disp).all():
            print("CP40: disp is not finite")
            a = 0
            b = 1
    ...
    def forward(self,
        ...
        disp = self.get_disp(pts, timestamps)

        return disp

    def get_disp_params(self):

        field_params = {k: v for k, v in self.disp_grids.named_parameters(prefix="disp_grids")}

        nn_params = [
                self.disp_net.named_parameters(prefix="disp_net")
                ]
        nn_params = {k: v for plist in nn_params for k, v in plist}
