...
from plenoxels.tr_ts_data_loading import init_ts_data
import pandas as pd
import torch
...
from .new_regularization import (
    PlaneTV, TimeSmoothness, HistogramLoss, L1TimePlanes, DistortionLoss,L2motionloss,L1TimePlanesRevgrid,Sparse_Flow_loss
)
...
class VideoTrainer(BaseTrainer):
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16), torch.no_grad():
            ...
            for b in range(math.ceil(rays_o.shape[0] / batch_size)):
                ...
                for k, v in outputs.items():
                    if "rgb" in k or "depth" in k or "t+10" in k or "t-10" in k:
    ...
    @torch.no_grad()
    def validate(self):
        # change dataset to required index
        test_dataset = init_ts_data(self.extra_args['data_dirs'][0], split='test', **self.extra_args)
        dataset = test_dataset['ts_dset']
    ...
    @torch.no_grad()
    def validate_new(self, required_frame):
        # change dataset to required index
        test_dataset = init_ts_data(self.extra_args['data_dirs'][0], split='test', **self.extra_args)
        dataset = test_dataset['ts_dset']
        #dataset = self.test_dataset
        ...
        if required_frame is not None:
            path = '/home/kapilchoudhary/Downloads/DKnerf/data/dynerf/Sparse_flow/sparse_flow_csvs'
            file = Path(os.path.join(path, f'{required_frame:04}_01_{required_frame+10:04}_16.csv'))
            p = pd.read_csv(file)
            b = preds["t+10"].reshape(1014, 1352, 2).numpy()
            c = []

            for i, j in zip(p['y1'], p["x1"]):
                c1 = b[int(i), int(j)]

                c.append(c1)
            c = numpy.stack(c)
            loss = np.linalg.norm([p["x2"], p["y2"]] - c.T)
            print(loss)
            b = pd.DataFrame(c)
            b.to_csv(
                "/home/kapilchoudhary/Downloads/DK_nerf_diff_config/plenoxels_with_3d_motion/logs/realdynamic/cutbeef_explicit/flow_0to10_fromvid1tovid16.csv")
            im = (255 * preds["rgb"].reshape(1014, 1352, 3).cpu().clamp(0, 1)).byte().numpy()
            a = Image.fromarray(im)
            a.save(
                "/home/kapilchoudhary/Downloads/DK_nerf_diff_config/plenoxels_with_3d_motion/logs/realdynamic/cutbeef_explicit/test.jpeg")
    ...
    def get_regularizers(self, **kwargs):
        return [
            ...
            L2motionloss(kwargs.get('L2_motion_loss', 0), what='r_field'),
            Sparse_Flow_loss(kwargs.get("optical_motion_loss", 0), what='optical_flow')
        ]

    # L1TimePlanesRevgrid(kwargs.get('l1_time_planes_rev_grid', 0.0), what='rev_field'), it didn't increase performance
