...
# from plenoxels.runners.new_regularization import L1motionloss
import numpy as np
import torch
...
class BaseTrainer(abc.ABC):
    def __init__(self,
        ...
        self.logger = SummaryWriter(log_dir=os.path.join(self.log_dir, "REMOVING_FP16"))
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        # def update_losses_data_(iter_num_: int, iter_losses_: dict, label: str): # TODO plotting loss tensorboard
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16):
            ...
            # Reconstruction loss
            losses = {}  #change TODO: change
            recon_loss = self.criterion(fwd_out['rgb'], data['imgs'])
            # Regularization
            losses["mse"] = [recon_loss]  # change
            self.logger.add_scalar("mse", recon_loss, global_step=self.global_step)
            loss = recon_loss
            # loss = loss + L1motionloss(fwd_out["rev_disp"], fwd_out["pts"])
            for r in self.regularizers:
                reg_loss = r.regularize(self.model, model_out=fwd_out)
                loss = loss + reg_loss
                losses[f"{r.reg_type}"] = reg_loss
                self.logger.add_scalar(r.reg_type, reg_loss, global_step=self.global_step)
                # TODO: KC add writer scalar
    ...
    def _move_data_to_device(self, data):
        ...
        data['w2c'] = data['w2c'].to(self.device)
        data["x"] = data["x"].to(self.device)
        data["y"] = data["y"].to(self.device)
        data["u"] = data["u"].to(self.device)
...
def init_dloader_random(_):
    ...
    # seed = 0  # TODO SNB: remove this seed
    np.random.seed(seed)
    random.seed(seed)
...
def initialize_model(
    ...
    if runner.train_dataset is not None:
        ...
        try:
            ...
            num_images = None
    model = LowrankModel(
        disp_grid_config=extra_args.pop("disp_grid_config"),
        model_grid_config=extra_args.pop("model_grid_config"),
