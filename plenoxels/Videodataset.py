...
class Video360Dataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            # TODO : make the csv files read without header ,CHANGE THE CSV DATA FROM FLOAT64 TO FLOAT32
            x1 = [torch.tensor(data[i]["x1"].values) for i in range(len(data))]
            no_of_samples_per_csv = [len(x1[i]) for i in range(len(data))]
            x1 = torch.cat(x1).to(torch.float32)
            y1 = torch.cat([torch.tensor(data[i]["y1"].values) for i in range(len(data))]).to(torch.float32)
            x2 = torch.cat([torch.tensor(data[i]["x2"].values) for i in range(len(data))]).to(torch.float32)
            y2 = torch.cat([torch.tensor(data[i]["y2"].values) for i in range(len(data))]).to(torch.float32)
            ...
            c = torch.cat([c2_id_rep[:int(csv_rand_points / 2)], c1_id_rep[:int(csv_rand_points / 2)]]).to(torch.float32)


            image_id1 = 300 * c1_id_rep + (f1_time+1)*self.timestamps_max/2
            image_id2 = 300 * c2_id_rep + (f2_time+1)*self.timestamps_max/2

            # adding csv and rand data # add randomness in this selection also
        ...
        # here the w2c is w2c for getting the ray direction and translation of the corresponding cameras
        ...
        if self.split == "train":
            ...
            #TODO: changed_at_oct11: 16:37
            '''c2w_for_checking = self.poses[im_idx1.long()]
            c2w_for_checking = torch.cat((c2w_for_checking, ones.repeat(c2w_for_checking.shape[0], 1, 1).to(torch.float32)), dim=1)
            torch.linalg.inv(c2w2)'''
            # ones = torch.tensor([0, 0, 0, 1])
            # ones = ones[None, ...]
            # c2w = torch.cat((c2w, ones.repeat(c2w.shape[0], 1, 1)), dim=1)
            ...
            # ones = torch.tensor([0, 0, 0, 1])
            # ones = ones[None, ...]
            c2w2 = torch.cat((c2w1, ones.repeat(c2w1.shape[0], 1, 1).to(torch.float32)), dim=1)  #check

            w2c = torch.linalg.inv(c2w2)  # changed on 20sep 12:09
            out['w2c'] = w2c  # change it again to w2c
            out["u"] = c2w2
            out["v"] = torch.cat([w2c[-int(csv_rand_points//2):], w2c[:int(csv_rand_points//2)]])
        #
        else:  # test
            ones = torch.tensor([0, 0, 0, 1])
            ones = ones[None, ...]
...
#  --log-dir logs/realdynamic/cutbeef_explicit/with_optical_motion_weight_0.00001 --validate-only
