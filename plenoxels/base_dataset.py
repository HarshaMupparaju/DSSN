...
class BaseDataset(Dataset, ABC):
    def __init__(self,
        ...
        self.ratio_of_optical_supervisied_train_data = ratio_of_optical_supervisied_train_data
        if self.split == 'train':
    ...
    def get_rand_ids(self, index):
        ...
        if self.sampling_weights is not None:
            ...
            if self.use_permutation:
                ...
                if self.ratio_of_optical_supervisied_train_data is not None and self.split == 'train':
                    a = int(batch_size * (1 - self.ratio_of_optical_supervisied_train_data))
                    if a < batch_size * (1 - self.ratio_of_optical_supervisied_train_data):
                        a = a+1
                    return torch.randint(0, self.num_samples, size=(a, ))
                else:
                    return torch.randint(0, self.num_samples, size=(batch_size,))

    def get_csv_rand_ids(self):
        min_no_of_opti_points = 30*2  # *2 because x2,y2 will also be the image pixels in cam2
        a = torch.randint(0, len(self.csv_data), size=(int((self.batch_size*self.ratio_of_optical_supervisied_train_data)/min_no_of_opti_points),))
        return a
